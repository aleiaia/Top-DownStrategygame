...
namespace Assets.Utils {
    ...
    public static class UtilsClass {
        ...
        public static bool CheckPointOnScreen(Vector3 point)
        {
            return CheckPointOnScreen(point, Camera.main);
        }

        public static bool CheckPointOnScreen(Vector3 point, Camera camera)
        {
            Vector3 viewPortSpacePoint = camera.WorldToViewportPoint(point);
            if (viewPortSpacePoint.z >= 0 && 
                viewPortSpacePoint.x > 0 && viewPortSpacePoint.y > 0 && 
                viewPortSpacePoint.x < 1 && viewPortSpacePoint.y < 1)
            {
                return true;
            }
            return false;
        ...
        public static bool CheckPointOnScreenLoose(Vector3 point, float tollerance = 0.01f)
        {
            return CheckPointOnScreenLoose(point, Camera.main, tollerance);
        }

        public static bool CheckPointOnScreenLoose(Vector3 point, Camera camera, float tollerance = 0.01f)
        {
            Vector3 viewPortSpacePoint = camera.WorldToViewportPoint(point);
            if (viewPortSpacePoint.z >= 0 &&
                viewPortSpacePoint.x > 0 - tollerance && viewPortSpacePoint.y > 0 - tollerance &&
                viewPortSpacePoint.x < 1 + tollerance && viewPortSpacePoint.y < 1 + tollerance)
            {
                return true;
            }
            return false;
